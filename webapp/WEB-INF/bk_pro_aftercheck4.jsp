...
<body>
	...
	<section id="main-content">
		...
		<form action="<%=basePath%>proo/lookquryList/1" id="frm1" class="form-horizontal tasi-form" method="post">
			<button type="submit" onclick="serch();" class="btn btn-theme03">查询</button>
			<div class="form-group">
						<label class="col-sm-2   control-label">理财产品名称:</label>
						<div class="col-sm-5">
							<input type="text" name="proname" value="${productinfo.proname }" class="form-control">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-2   control-label">产品投标期限:</label>
						<div class="col-sm-5">
							<c:if test="${productinfo.proday==0 }">
								<input type="text" name="proday" value=" "  class="form-control">
							</c:if>
							<c:if test="${productinfo.proday !=0 }">
								<input type="text" name="proday" value="${productinfo.proday }" class="form-control">
							</c:if>
						</div>
					</div>
					<input type="hidden" name="comcorid" value="1">
		</form>
