...
<body>
	...
	<section id="main-content">
		<section class="wrapper">
			...
			<div class="showback">
				<h4>
					<i class="fa fa-angle-right"></i> 所有理财
				</h4>
				<!-- 页面内容 -->
				<form id="frm1" class="form-horizontal tasi-form" action="<%=basePath%>fin/queryFinaList/1" method="post">
					<button type="submit"  class="btn btn-theme03">查找</button>
					<div class="form-group">
						<label class="col-sm-2   control-label">用户昵称:</label>
						<div class="col-sm-5">
							<input type="text" name="financeinfo.usernickname" value="${financeinfo.usernickname}" class="form-control">
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-2  control-label"> 理财编号：</label>
						<div class="col-sm-5">
							<c:if	test="${financeinfo.finid==0}">
								<input type="text" name="financeinfo.finid" value="" class="form-control">
							</c:if>
							<c:if	test="${financeinfo.finid!=0}">
								<input type="text" name="financeinfo.finid" value="${financeinfo.finid}" class="form-control">
							</c:if>
						</div>
					</div>
				</form>
			...
			<div class="showback">
				...
				<table class="table table-hover">
					<thead>
						...
						<c:if test="${not empty finaList  }">
						<tbody>
						<c:forEach items="${finaList }" var="fl" varStatus="status">
							...
							<tr>
								...
								<td>
									<a href="<%=basePath%>fin/queryFinaOne/${fl.finid}/1">审核投标信息</a>
								</td>
							</tr>
					...
					<tfoot>
							<tr align="center">
								<td colspan="11">
									<a href="<%=basePath%>fin/queryFinaList/${pageInfo.getFirstPage()}" class="btn btn-success btn-ms ">首页</a>
									<c:if test="${pageNum>1 }">	
									<a href="<%=basePath%>fin/queryFinaList/${pageNum-1 }" class="btn btn-success btn-ms ">&lt;&lt;上一页</a>
									</c:if>	
									<c:if test="${pageNum<pageInfo.getLastPage() }">
									<a href="<%=basePath%>fin/queryFinaList/${pageNum+1 }" class="btn btn-success btn-ms ">下一页&gt;&gt;</a>
									</c:if>
									<a href="<%=basePath%>fin/queryFinaList/${pageInfo.getLastPage() }" class="btn btn-success btn-ms "> 尾页</a>
						    		<font size="3">
