					...
					<tbody>
						<c:forEach items="${list}" var="n" varStatus="status">
							<tr>
								...
								<td>${n.proprice}</td>
								<td>${n.proprofit}</td>
								<td><a href="<%=basePath%>proo/queryDetail/${n.proid}/1" data-target="#${status.index+1}"
									>待审核</a></td>
						</c:forEach>
						<tr align="center">
							<td colspan="13"><a
								href="${pageContext.request.contextPath}/proo/productList/1"
								class="btn btn-success btn-ms ">首页</a> <a
								href="${pageContext.request.contextPath}/proo/productList/${p.prePage}"
								class="btn btn-success btn-ms ">&lt;&lt;上一页</a> <a
								href="${pageContext.request.contextPath}/proo/productList/${p.nextPage}"
								class="btn btn-success btn-ms "> 下一页&gt;&gt;</a> <a
								href="${pageContext.request.contextPath}/proo/productList/${p.pages}"
