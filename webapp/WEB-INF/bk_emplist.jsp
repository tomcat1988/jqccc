			...
			<div class="col-md-12 mt">
				<div class="content-panel">
					<h4>
						...
						<i class="fa fa-angle-right">
							<a  href="<%=basePath%>emp/toAddEmp">添加员工</a>
						</i>
						<br/><br/>
						<form action="<%=basePath%>emp/selectEmp/1" method="post">
							<span>用户名：</span>
							<input type="text" name="emp.likeName" value="${likeName }" maxlength="10" />
					...
					<table class="table table-hover">
						...
						<tbody>
							<c:forEach items="${el }" var="list" varStatus="sta">
								<tr>
									<td>${sta.index+1 }</td>
									<td>${list.empid }</td>
									<td>${list.empusername }</td>
									<td>${list.emppwd }</td>
									<td>${list.empname }</td>
									<td>${list.power }</td>
									<td>
										<a href="<%=basePath%>emp/toUpdateEmp/${list.empid}">编辑</a>
										<a href="<%=basePath%>emp/deleteEmp/${list.empid}">删除</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
						<tbody>
							<tr>
								...
								<c:choose>
									<c:when test="${el!=null }">
										<a href="<%=basePath%>emp/selectEmp/${p.getFirstPage()}"
											class="btn btn-success btn-ms ">首页</a> 
										<c:if test="${pageNum>1}">
										<a href="<%=basePath%>emp/selectEmp/${pageNum-1}"
											class="btn btn-success btn-ms ">&lt;&lt;上一页</a> 
										</c:if>
										<c:if test="${pageNum<p.getLastPage()}">
										<a href="<%=basePath%>emp/selectEmp/${pageNum+1}"
											class="btn btn-success btn-ms "> 下一页&gt;&gt;</a>
										</c:if>
										<a href="<%=basePath%>emp/selectEmp/${p.getLastPage()}"
											class="btn btn-success btn-ms "> 尾页</a>
										<font size="3">共<font color="red">${p.getTotal()}</font>条数据，当前<font color="#0000ff">${p.getPageNum()}</font><font
												color="red">/${p.getLastPage()}</font>页
										</font>
									</c:when>
						 			<c:otherwise>
